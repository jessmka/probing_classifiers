...
class MappingDicts:
    def __init__(self, folder):
        ...
        author_gender = pd.read_csv(os.path.join(folder, 'final_dataset.csv'))
        self.author_gender = author_gender[['gender']].to_dict('index')
...
class LLMRecs:
    ...
    def author_gender(self, small_df):
        ...
        for typ in ('baseline','steered'):
            ...
            small_df[f'{typ}_gender_list'] = small_df[f'typ_author_list'].apply(
                lambda row: [
    ...
    def build_full_df(self):
        ...
        mfrec = MFRec(mdf, 'goodreads_data')
        preds_df, user_id_map = mfrec.do_mf()

        self.map_titles(outputs_df)
        self.count_from_hist(outputs_df)
        self.author_gender(outputs_df)
        self.user_gender(outputs_df)
        outputs2 = self.lookup_mf_scores(outputs_df, preds_df, user_id_map)
    ...
    def append_mf_to_df(self, small_df, outputs_dict):
        for typ in ('baseline','steered'):
            small_df[f'mf_{typ}_scores'] = small_df['user_id'].apply(lambda x: outputs_dict.get(x).get(f'mf_{typ}_score'))
            small_df[f'mf_{typ}_denom'] = small_df['user_id'].apply(lambda x: outputs_dict.get(x).get(f'mf_{typ}_denom'))
    ...
    def output_mf_scores(self, df):
        ...
        for typ in ('baseline','steered'):
            ...
            print(f'{typ} - MF score by gender',result['sum_value1']/result['sum_value2'])
...
class MFRec:
    def __init__(self, df, foldername, num_users_baseline=10):
        self.map_dicts = MappingDicts(foldername)
        ...
        self.user_ids_fromdf = int_df[int_df.is_read == 1].user_id.unique()
    ...
    def do_mf(self):
        ...
        return preds_df, self.user_ids_fromdf
...
if __name__ == '__main__':
    ...
    llm_recs = LLMRecs('goodreads_data', 1000)
